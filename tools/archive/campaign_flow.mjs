...
function matchPersonas(personas, text) {
  const fuse = new Fuse(personas, { keys: ['category', 'description', 'audience'] });
  return fuse.search(text).slice(0, 3).map(r => r.item);
}
...
function main() {
  ...
  const emails = matches.map(p => createEmailTemplate(brand, p)).join('\n---\n');
  fs.writeFileSync('sample_emails.txt', emails);
  console.log('Generated campaign_brief.pdf and sample_emails.txt');
}
