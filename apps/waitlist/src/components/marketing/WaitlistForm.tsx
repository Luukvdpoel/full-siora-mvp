...
// Supabase integration can be connected later for persistence
...
const WaitlistForm = () => {
  ...
  const onSubmit = async (values: FormValues) => {
    setLoading(true);
    try {
      // Persist locally for now; backend hookup (Supabase) can be added later
      try {
      ...
      toast.success("You're on the list — we'll be in touch soon!", { description: "Thanks for your interest in Siora." });
      ...
      toast("You're on the list!", { description: "We saved your interest locally — backend will sync soon." });
    } finally {
      setLoading(false);
    }
  };
    ...
    <section id="waitlist" className="relative">
      <div className="container mx-auto px-6 py-16 md:py-24">
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="rounded-xl border border-[hsl(var(--brand)/0.2)] bg-card/60 p-6 shadow-sm backdrop-blur">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 mt-1 flex items-center gap-2">
                <input id="consent" type="checkbox" className="size-4 accent-[hsl(var(--brand))]" {...form.register("consent")} />
                <Label htmlFor="consent" className="text-sm text-muted-foreground">I agree to receive early access updates</Label>
              </div>
